...
void insertHeapNode(Heap *heap, void *data){
  ...
  else
  {
    ...
    {
      if (i+2 >= heap->size) /* +2 ensures last element is NULL */
      {
      /*  out of space*/

        heap->arr = realloc(heap->arr, sizeof(HeapNode *)*(heap->size+5));
...
void changeHeapType(Heap *heap){
    ...
    localHeap = createHeap(10, heap->type, &destroyData, NULL, &compare);
...
void heapify (Heap *heap, HeapNode * newNode){
  ...
  int i = 0;
  int pos = 0;
  client *one = (client *)(newNode->data);
  client *two = (client *)(heap->arr[pos]->data);
  client *swapped;

  while (strcmp(one->ID, two->ID) != 0)
  {
    pos++;
    two = (client *)heap->arr[pos]->data;
  }
  ...
  else
  {
    while (heap->compare(newNode->data, heap->arr[parentNodeindex]->data) == -1) /*min heap */
    {
      ...
      heap->arr[parentNodeindex] = newNode;
      heap->arr[pos] = temp;
      swapped = (client *)(heap->arr[pos]->data);
      if (swapped->priority > 1)
      {
        swapped->priority = swapped->priority - 1;
      }

     heap->arr[pos]->data = (void *)(swapped);
      pos = parentNodeindex;
      parentNodeindex = (pos - 1)/2;
    }
  }
